...
function etiquetaTipo(tipo: string): { texto: string; color: string } {
  if (tipo.startsWith("alerta_")) return { texto: "ALERTA DE CRECIDA", color: "#C0442B" };
  if (tipo.startsWith("aviso_")) return { texto: "AVISO DE CRECIDA", color: "#E8823A" };
  if (tipo.startsWith("cese_")) return { texto: "CESE DE AVISO", color: "#4C7A5E" };
  return { texto: tipo.toUpperCase().replace(/_/g, " "), color: "#C99A3D" };
}
...
export default function AvisoCrecidaCard({ aviso, umbralNR }: Props) {
  ...
  return (
    <section className="dashboard-section" style={{ borderColor: `${etiqueta.color}66`, borderLeft: `4px solid ${etiqueta.color}` }}>
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        <p className="seccion-titulo" style={{ color: etiqueta.color }}>
          {etiqueta.texto}
        </p>
        {aviso.emitido && (
          <span className="text-xs text-[#5B6E68]/70 dark:text-gray-500">
            {fmtEmitido(aviso.emitido)}
          </span>
        )}
      </div>

      <p className="text-sm font-medium text-[#12312B] dark:text-gray-200 mb-1">
        {aviso.titulo}
      </p>
      <p className="text-sm text-[#5B6E68] dark:text-gray-400 mb-3 leading-snug">
        {aviso.texto}
      </p>
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between text-sm rounded-lg px-3 py-1.5 ${
                    esSF ? "bg-[#0E4749]/10 dark:bg-white/10 font-bold" : ""
                  }`}
                >
                  <span className="text-[#12312B] dark:text-gray-200">
                    {a.puerto}
                  </span>
                  <span className={`font-mono ${esSF ? "text-[#0E4749] dark:text-[#4fc3c5]" : "text-[#5B6E68] dark:text-gray-400"}`}>
      ...
      {superaNR && (
        <div className="flex items-center gap-2 text-[#C0442B] dark:text-[#E5604A] font-bold text-sm mb-3">
      ...
      {aviso.nota && (
        <p className="text-xs text-[#5B6E68]/70 dark:text-gray-500 mb-3 italic">
          {aviso.nota}
        </p>
      )}
