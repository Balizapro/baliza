...
function labelError(errorCm: number | null): { texto: string; color: string } {
  if (errorCm == null) return { texto: "--", color: "text-texto-sec dark:text-gray-400" };
  const abs = Math.abs(errorCm);
  if (abs <= 10) return { texto: "Acierto alto", color: "text-ok border-ok/50 bg-ok/10" };
  if (abs <= 30) return { texto: "Aproximado", color: "text-atencion border-atencion/50 bg-atencion/10" };
  return { texto: "Desviado", color: "text-rojo-alerta border-rojo-alerta/50 bg-rojo-alerta/10" };
}
  ...
  if (!ultimo) {
    return (
      <section className="dashboard-section">
        <h2 className="seccion-titulo mb-2">Verificación de pronóstico</h2>
  ...
  return (
    <section className="dashboard-section">
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        <h2 className="seccion-titulo">Verificación de pronóstico</h2>
      ...
      <p className="text-sm text-texto-sec dark:text-gray-400 mb-3">
        Ciclo emitido <span className="font-medium text-baliza dark:text-marea-dark">{fmtDia(ultimo.fecha)}</span> —
        qué tan bien anticipó el pico en San Fernando:
      </p>

      <div className="grid grid-cols-3 gap-3 text-center mb-4">
        <div className="rounded-lg bg-fondo/50 dark:bg-white/5 p-3">
          <p className="text-xs uppercase tracking-wide text-texto-sec dark:text-gray-400">Pronosticado</p>
          <p className="font-mono text-lg font-bold text-baliza dark:text-marea-dark">
            {ultimo.pico.valor_m.toFixed(2)}m
          </p>
          <p className="text-xs text-texto-sec dark:text-gray-400">{fmtFecha(ultimo.pico.timestamp)}</p>
        </div>
        <div className="rounded-lg bg-fondo/50 dark:bg-white/5 p-3">
          <p className="text-xs uppercase tracking-wide text-texto-sec dark:text-gray-400">Observado</p>
          <p className="font-mono text-lg font-bold text-baliza dark:text-marea-dark">
        ...
        <div className="rounded-lg bg-fondo/50 dark:bg-white/5 p-3">
          <p className="text-xs uppercase tracking-wide text-texto-sec dark:text-gray-400">Error</p>
          <p className={`font-mono text-lg font-bold ${ultimo.errorCm == null || Math.abs(ultimo.errorCm) <= 30 ? "text-ok" : "text-rojo-alerta"}`}>
      ...
      {recientes.length > 1 && (
        <>
          <div className="flex flex-wrap gap-3 mb-3">
            {errorPromedio != null && (
              <span className="text-xs px-2 py-1 rounded-md bg-fondo/50 dark:bg-white/5 text-texto dark:text-gray-300">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs uppercase tracking-wide text-texto-sec dark:text-gray-400 border-b border-baliza/10 dark:border-white/10">
              ...
              <tbody>
                {recientes.map((c) => {
                  ...
                  return (
                    <tr key={c.fecha} className="border-b border-baliza/5 dark:border-white/5">
                      <td className="py-1.5 pr-2 text-texto dark:text-gray-300">{fmtDia(c.fecha)}</td>
                      <td className="py-1.5 px-2 text-right font-mono text-baliza dark:text-marea-dark">{c.pico.valor_m.toFixed(2)}m</td>
                      <td className="py-1.5 px-2 text-right font-mono">{c.picoObs?.nivel_m.toFixed(2)}m</td>
                      <td className={`py-1.5 pl-2 text-right font-mono font-medium ${Math.abs(e) <= 30 ? "text-ok" : "text-rojo-alerta"}`}>
                        {e >= 0 ? "+" : ""}{e}cm
                      </td>
                    </tr>
                  );
